...
testthat::test_that("Lengths and DIMs check", {
    testthat::expect_length(
        IMSSLAER::GMRES(A = AN, u = uN, f = fN, eps = eps[1]), 
        n = length(uN)
        )
    for (e in eps) {
        for (i in 1:(Reductor + 1)) {
            testthat::expect_error(
                IMSSLAER::GMRES(A = AN, u = uN, 
                                f = fN[-i], eps = e)
                )
            testthat::expect_error(
                IMSSLAER::GMRES(A = AN, u = uN[-i], 
                                f = fN[-i], eps = e)
                )
            testthat::expect_error(
                IMSSLAER::GMRES(A = AN, u = uN[-i], 
            ...
            testthat::expect_error(
                IMSSLAER::GMRES(A = AN[-i, ], u = uN[-i], 
                                f = fN, eps = e)
                )
            testthat::expect_error(
                IMSSLAER::GMRES(A = AN[-i, ], u = uN, 
                                f = fN, eps = e)
                )
            testthat::expect_error(
                IMSSLAER::GMRES(A = AN[-i, ], u = uN, 
...
testthat::test_that("GMRES works", {
    ...
    testthat::expect_length(
        IMSSLAER::GMRES(M, fC, uN, eps = 10e-6, 
                        iterations = 10000), 
        n = length(uN)
        )
})
