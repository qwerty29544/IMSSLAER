...
testthat::test_that("Accuracy and numeric/complex type check", {
    for (e in eps) {
        ...
        testthat::expect_true(abs((sqrt(t(AC %*% result - fN) %*% Conj(AC %*% result - fN)))/(sqrt(t(fN) %*% Conj(fN)))) < e)
        # N/C/C
        result <- IMSSLAER::GDM(A = AN, f = fC, u = uC, eps = e, iterations = 40000)
        testthat::expect_true(abs((sqrt(t(AN %*% result - fC) %*% Conj(AN %*% result - fC)))/(sqrt(t(fC) %*% Conj(fC)))) < e)
        # C/C/C
...
testthat::test_that("Lengths and DIMs check", {
    testthat::expect(IMSSLAER::GDM(A = AN, u = uN, f = fN, eps = eps[1]))
    for (e in eps) {
        for (i in 1:(Reductor + 1)) {
            testthat::expect_error(IMSSLAER::GDM(A = AN, u = uN, f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN, u = uN[-i], f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN, u = uN[-i], f = fN, eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN[-i, ], u = uN[-i], f = fN, eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN[-i, ], u = uN, f = fN, eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN[-i, ], u = uN, f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN[-i, ][, -i], u = uN, f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN[-i, ][, -i], u = uN[-i], f = fN, eps = e))
            testthat::expect_error(IMSSLAER::GDM(A = AN[-i, ], u = uN[-i], f = fN[-i], eps = e))
            testthat::expect(IMSSLAER::GDM(A = AN[-i, ][, -i], u = uN[-i], f = fN[-i], eps = e))
        }
    }
})
...
testthat::test_that("GDM works", {
    ...
    testthat::expect_length(IMSSLAER::GDM(M, fC, uN, eps = 10e-6, iterations = 10000), n = Reductor + 1)
})
