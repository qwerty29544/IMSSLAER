...
testthat::test_that("Accuracy and numeric/complex type check", {
    for (e in eps) {
        # N/N/N
        result <- IMSSLAER::Chebishev(A = AN, f = fN, u = uN, 
                                      eps = e, layers = 5, 
                                      lambs = diag(AN))
        ...
        # N/N/C
        result <- IMSSLAER::Chebishev(A = AN, f = fN, u = uC, 
                                      eps = e, layers = 5, 
                                      lambs = diag(AN))
        testthat::expect_true(
            abs((sqrt(t(AN %*% result - fN) %*% 
        ...
        # N/C/N
        result <- IMSSLAER::Chebishev(A = AN, f = fC, u = uN, 
                                      eps = e, layers = 5, 
                                      lambs = diag(AN))
        testthat::expect_true(
        ...
        testthat::expect_error(
            IMSSLAER::Chebishev(A = AC, f = fN, u = uN, 
                                eps = e, lambs = diag(AC), 
                                layers = 5)
            )
        # C/C/N
        testthat::expect_error(
            IMSSLAER::Chebishev(A = AC, f = fC, u = uN, 
                                eps = e, lambs = diag(AC), 
                                layers = 5)
            )
        # C/N/C
        testthat::expect_error(
            IMSSLAER::Chebishev(A = AC, f = fN, u = uC, 
                                eps = e, lambs = diag(AC), 
                                layers = 5)
            )
        # N/C/C
        result <- IMSSLAER::Chebishev(A = AN, f = fC, u = uC, 
                                      eps = e, layers = 5, 
                                      lambs = diag(AN)
                                      )
        testthat::expect_true(
        ...
        # C/C/C
        testthat::expect_error(
            IMSSLAER::Chebishev(A = AC, f = fC, u = uC, 
                                eps = e, lambs = diag(AC), 
                                layers = 5)
            )
    }
...
testthat::test_that("Lengths and DIMs check", {
    ...
    for (e in eps) {
        for (i in 1:(Reductor + 1)) {
            ...
            testthat::expect_error(
                IMSSLAER::Chebishev(A = AN, u = uN[-i], 
                                    f = fN[-i], eps = e, 
                                    layers = 5, 
                                    lambs = diag(AN))
                )
            ...
            testthat::expect_error(
                IMSSLAER::Chebishev(A = AN[-i, ], u = uN[-i], 
                                    f = fN, eps = e, 
                                    layers = 5, 
                                    lambs = diag(AN))
                )
            testthat::expect_error(
                IMSSLAER::Chebishev(A = AN[-i, ], u = uN, 
                                    f = fN, eps = e, 
                                    layers = 5, 
                                    lambs = diag(AN))
                )
            ...
            testthat::expect_error(
                IMSSLAER::Chebishev(A = AN[-i, ][, -i], u = uN, 
                                    f = fN[-i], eps = e, 
                                    layers = 5, 
                                    lambs = diag(AN))
                )
            testthat::expect_error(
                IMSSLAER::Chebishev(A = AN[-i, ][, -i], 
                                    u = uN[-i], f = fN, eps = e, 
                                    layers = 5, 
                                    lambs = diag(AN))
                )
...
testthat::test_that("Error stuctures check", {
    ...
    for (e in eps) {    
        M <- rnorm(Reductor + 1)
        testthat::expect_error(
            IMSSLAER::Chebishev(M, fN, uN, e, layers = 5, 
                                lambs = diag(AN)) 
            )
        M <- list(vec = rnorm(Reductor + 1))
        testthat::expect_error(
            IMSSLAER::Chebishev(AN, M, uN, e, layers = 5, 
                                lambs = diag(AN))
            )
        testthat::expect_error(
            IMSSLAER::Chebishev(AN, fN, M, e, layers = 5, 
                                lambs = diag(AN))
            )
        testthat::expect_error(
            IMSSLAER::Chebishev(M, fN,  uN, e, layers = 5, 
...
testthat::test_that("Chebishev works", {
    ...
    testthat::expect_error(
        IMSSLAER::Chebishev(M, fC, uN, eps = 10e-6, 
                            iterations = 10000, layers = 5, 
                            lambs = diag(AN))
        )
})
