...
testthat::test_that("Lengths and DIMs check", {
    testthat::expect(IMSSLAER::BiCGM(A = AN, u = uN, f = fN, eps = eps[1]))
    for (e in eps) {
        for (i in 1:(Reductor + 1)) {
            testthat::expect_error(IMSSLAER::BiCGM(A = AN, u = uN, f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN, u = uN[-i], f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN, u = uN[-i], f = fN, eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN[-i, ], u = uN[-i], f = fN, eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN[-i, ], u = uN, f = fN, eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN[-i, ], u = uN, f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN[-i, ][, -i], u = uN, f = fN[-i], eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN[-i, ][, -i], u = uN[-i], f = fN, eps = e))
            testthat::expect_error(IMSSLAER::BiCGM(A = AN[-i, ], u = uN[-i], f = fN[-i], eps = e))
            testthat::expect(IMSSLAER::BiCGM(A = AN[-i, ][, -i], u = uN[-i], f = fN[-i], eps = e))
        }
    }
})
...
testthat::test_that("BiCGM works", {
    ...
    testthat::expect_message(IMSSLAER::BiCGM(M, fC, uN, eps = 10e-6, iterations = 10000))
})
